...
router.post('/', authenticateToken, async (req: AuthRequest, res) => {
  try {
    ...
    const result = await query(`
      ...
      [schoolId,d.busNumber,d.plateNumber,d.driverName,d.driverPhone,d.supervisorName,
       d.supervisorPhone,d.capacity||40,d.routeName,d.routeDescription,d.morningTime,
       d.afternoonTime,d.isActive!==false,d.gpsId,d.notes]
    )
...
router.put('/:id', authenticateToken, async (req: AuthRequest, res) => {
  try {
    ...
    const result = await query(`
      ...
      [d.busNumber,d.plateNumber,d.driverName,d.driverPhone,d.supervisorName,d.supervisorPhone,
       d.capacity,d.routeName,d.routeDescription,d.morningTime,d.afternoonTime,d.isActive,
       d.gpsId,d.notes,req.params.id,req.user!.schoolId]
    )
...
router.get('/:id/students', authenticateToken, async (req: AuthRequest, res) => {
  try {
    const result = await query(`SELECT * FROM students WHERE bus_id=$1 ORDER BY name`, [req.params.id])
    res.json({ students: result.rows })
  } catch { res.status(500).json({ error: 'Server error' }) }
})

router.post('/:id/students', authenticateToken, async (req: AuthRequest, res) => {
  try {
...
router.delete('/:id/students/:studentId', authenticateToken, async (req: AuthRequest, res) => {
  try {
